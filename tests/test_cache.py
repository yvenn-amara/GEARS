...
def test_load_or_run_benchmark_cache_hit_skips_rerun(tmp_path, monkeypatch):
    ...
    save_cache(cached_results, config, cache_dir=tmp_path)
    ...
    results, from_cache = load_or_run_benchmark(
        datasets={}, config=config, force_rerun=False, cache_dir=tmp_path,
    )
...
def test_load_or_run_benchmark_force_rerun_bypasses_cache(tmp_path, monkeypatch):
    ...
    save_cache(stale_cached, config, cache_dir=tmp_path)
    ...
    # in-memory), so a later cache_hit test with the same config sees it.
    reloaded = load_cached(config, cache_dir=tmp_path)
...
def test_load_or_run_benchmark_cache_miss_runs_and_caches(tmp_path, monkeypatch):
    ...
    assert load_cached(config, cache_dir=tmp_path) is None  # sanity: real miss
    results, from_cache = load_or_run_benchmark(
        datasets={"office": pd.DataFrame()}, config=config,
    ...
    assert from_cache is False
    pd.testing.assert_frame_equal(results, fresh_results)
    # Second call with the identical config must now hit the cache.
