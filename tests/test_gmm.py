...
def _mean_feature(gmm_obj: EVSessionGMM, ctx: tuple, feature_idx: int) -> float:
...
def test_recency_none_matches_pristine_unmodified_code():
    ...
    EVSessionGMM.fit()/._fit_single() that existed before recency weighting
...
def test_recency_none_equals_default_omitted():
    ...
    gmm_a = EVSessionGMM(n_components=3, random_state=5, recency=None).fit(df)
    gmm_b = EVSessionGMM(n_components=3, random_state=5).fit(df)
...
def test_recency_default_cap_is_5000():
    ...
    from gears.models.gmm import DEFAULT_RECENCY_RESAMPLE_CAP
...
def test_recency_ignored_for_vae_with_warning(caplog):
    ...
    df = make_df_vae(300)
    with caplog.at_level(logging.WARNING, logger="gears.models.gmm"):
        model = EVSessionGMM(
            model_type="vae",
